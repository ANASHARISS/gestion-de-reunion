...
const closeBtn = document.getElementById('close-modal-create');
if(closeBtn) {
...
function ajouterReunion(reunion) {
    reunion.id = Date.now(); // ID unique
    reunion.dateCreation = new Date().toISOString();
    reunion.statut = 'active'; // Statut par défaut
    reunions.push(reunion);
    sauvegarderReunions();
    mettreAJourAccueil();
    mettreAJourPlanification();
    mettreAJourTableauAdmin();
...
function mettreAJourAccueil() {
    ...
    // Compter les réunions créées ce mois
    const reunionsCeMois = reunions.filter(r => new Date(r.dateCreation) >= debutMois && r.statut === 'active').length;
    
    // Compter les réunions à venir cette semaine
    const reunionsAVenir = reunions.filter(r => {
        const dateReunion = new Date(r.date + 'T' + r.heure);
        return dateReunion >= maintenant && dateReunion <= new Date(maintenant.getTime() + 7 * 24 * 60 * 60 * 1000) && r.statut === 'active';
    }).length;
    
    // Dernière activité
    const derniereActivite = reunions.filter(r => r.statut === 'active').length > 0 ? reunions.filter(r => r.statut === 'active')[reunions.filter(r => r.statut === 'active').length - 1] : null;
    
    // Mettre à jour l'affichage
    const accueilSection = document.getElementById('accueil');
    if(accueilSection) {
        const statsContainer = accueilSection.querySelector('.welcome-card');
        if(statsContainer) {
            // Mettre à jour les statistiques
            const statsDivs = statsContainer.querySelectorAll('.welcome-card > div:nth-child(3) > div');
            if(statsDivs.length >= 3) {
                statsDivs[0].querySelector('div:nth-child(3)').textContent = reunionsCeMois;
                statsDivs[1].querySelector('div:nth-child(3)').textContent = reunionsAVenir;
            }
            
            // Mettre à jour la dernière activité
            const activiteDiv = statsContainer.querySelector('.welcome-card > div:nth-child(4)');
            if(activiteDiv && derniereActivite) {
                const dateCreation = new Date(derniereActivite.dateCreation).toLocaleDateString('fr-FR');
                activiteDiv.innerHTML = `
                    ...
                    <div style="color:#64748b; line-height:1.5;">
                        <div>• <strong>${derniereActivite.titre}</strong> créée le ${dateCreation}</div>
                        <div>• ${reunions.filter(r => r.statut === 'active').length} réunion(s) active(s)</div>
                    </div>
                `;
            }
        }
...
function mettreAJourPlanification() {
    const planifSection = document.getElementById('planification');
    if(planifSection) {
        const meetingList = planifSection.querySelector('.meeting-list');
        if(meetingList) {
            ...
            reunionsAVenir.forEach(reunion => {
                const dateReunion = new Date(reunion.date + 'T' + reunion.heure);
                ...
                meetingCard.innerHTML = `
                    ...
                    <div class="meeting-card-content">
                        <div class="meeting-title-row">
                            <span class="meeting-title"><b>${reunion.titre}</b></span>
                            <span style="margin-left:auto; color:#64748b;">${dateFormatee}</span>
                        </div>
                        <div class="meeting-details">${reunion.type} · ${reunion.description || 'Aucune description'}</div>
                        <div class="meeting-actions">
                            <a href="#" class="meeting-link">Voir</a>
                            <button class="btn-copy" onclick="annulerReunion(${reunion.id})">Annuler</button>
                        </div>
                    </div>
                `;
...
function mettreAJourHistorique() {
    const reunionsSection = document.getElementById('reunions');
    if(reunionsSection) {
        const tableBody = reunionsSection.querySelector('tbody');
        if(tableBody) {
            tableBody.innerHTML = '';
            
            // Récupérer toutes les réunions (actives et annulées) triées par date
            const toutesReunions = [...reunions].sort((a, b) => new Date(b.dateCreation) - new Date(a.dateCreation));
            
            toutesReunions.forEach(reunion => {
                const dateReunion = new Date(reunion.date + 'T' + reunion.heure);
                const dateFormatee = dateReunion.toLocaleDateString('fr-FR', { 
                    day: '2-digit', 
                    month: '2-digit', 
                ...
                row.innerHTML = `
                    <td>${reunion.titre}</td>
                    <td>${dateFormatee}</td>
                    <td><span style="color:${statutColor}; font-weight:bold;">${statutText}</span></td>
                    <td><a href="#" class="meeting-link">Voir les détails</a></td>
                `;
...
function annulerReunion(id) {
    ...
    if(reunion) {
        reunion.statut = 'annulée';
        reunion.dateAnnulation = new Date().toISOString();
    }
    sauvegarderReunions();
    mettreAJourAccueil();
    mettreAJourPlanification();
...
document.getElementById('form-create-meeting').addEventListener('submit', function(e) {
    e.preventDefault();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    mettreAJourTableauAdmin();
    mettreAJourHistorique();
    initialiserCentreAdmin();
    mettreAJourStatistiques();
    initialiserAide();
    initialiserDeconnexion();
});

// --- Centre admin - Gestion des utilisateurs ---
let utilisateurs = JSON.parse(localStorage.getItem('utilisateurs')) || [
    { id: 1, nom: 'Sarah Dupont', email: 'sarah.dupont@email.com', role: 'Admin' },
    { id: 2, nom: 'Ali Benali', email: 'ali.benali@email.com', role: 'Utilisateur' }
];
...
// Ajout utilisateur via le formulaire principal
    ...
    if(formAjout) {
        formAjout.addEventListener('submit', function(e) {
            ...
            if(nom && email && role) {
                ...
                mettreAJourTableauUtilisateurs();
                formAjout.reset();
            }
        });
    }
...
function ouvrirModalAjoutUtilisateur() {
    // Supprimer tout ancien modal avant d'en créer un nouveau
    const oldModal = document.getElementById('modal-ajout-user');
    if(oldModal) oldModal.remove();
    const modal = document.createElement('div');
    modal.id = 'modal-ajout-user';
    modal.style.cssText = 'display:flex; position:fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(0,0,0,0.25); z-index:1000; align-items:center; justify-content:center;';
    modal.innerHTML = `
        <div style="background:var(--card-bg,#fff); border-radius:18px; box-shadow:0 4px 24px var(--shadow-color,#0002); padding:36px 32px; min-width:320px; max-width:90vw; margin:auto;">
            <h2 style="color:var(--text-accent,#fbbf24); margin-top:0;">Ajouter un utilisateur</h2>
            <form id="form-ajout-user-modal">
                <div style="margin-bottom:14px;">
                    <label for="nom-user" style="font-weight:600; color:var(--text-primary,#222);">Nom complet</label><br>
                    <input type="text" id="nom-user" required style="width:100%; padding:8px 10px; border-radius:8px; border:1.5px solid var(--border-color,#fbbf24); background:var(--input-bg,#fffbe6); color:var(--text-primary,#222);">
                </div>
                <div style="margin-bottom:14px;">
                    <label for="email-user" style="font-weight:600; color:var(--text-primary,#222);">Email</label><br>
                    <input type="email" id="email-user" required style="width:100%; padding:8px 10px; border-radius:8px; border:1.5px solid var(--border-color,#fbbf24); background:var(--input-bg,#fffbe6); color:var(--text-primary,#222);">
                </div>
                <div style="margin-bottom:18px;">
                    <label for="role-user" style="font-weight:600; color:var(--text-primary,#222);">Rôle</label><br>
                    <select id="role-user" required style="width:100%; padding:8px 10px; border-radius:8px; border:1.5px solid var(--border-color,#fbbf24); background:var(--input-bg,#fffbe6); color:var(--text-primary,#222);">
                        <option value="Utilisateur">Utilisateur</option>
                        <option value="Admin">Admin</option>
                    </select>
                </div>
                <div style="text-align:right;">
                    <button type="button" onclick="fermerModalAjoutUtilisateur()" class="btn-annuler">Annuler</button>
                    <button type="submit" class="btn-create"><i class="fa fa-check"></i> Ajouter</button>
                </div>
            </form>
        </div>
    `;
    ...
    const form = modal.querySelector('#form-ajout-user-modal');
    form.onsubmit = function(e) {
        e.preventDefault();
        const nom = document.getElementById('nom-user').value.trim();
...
function modifierUtilisateur(id) {
    ...
    const modal = document.createElement('div');
    modal.id = 'modal-modif-user';
    modal.style.cssText = 'display:flex; position:fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(0,0,0,0.25); z-index:1000; align-items:center; justify-content:center;';
    
    modal.innerHTML = `
        <div style="background:var(--card-bg); border-radius:18px; box-shadow:0 4px 24px var(--shadow-color); padding:36px 32px; min-width:320px; max-width:90vw; margin:auto;">
            <h2 style="color:var(--text-accent); margin-top:0;">Modifier l'utilisateur</h2>
            <form id="form-modif-user">
                <div style="margin-bottom:14px;">
                    <label for="nom-modif" style="font-weight:600; color:var(--text-primary);">Nom complet</label><br>
                    <input type="text" id="nom-modif" value="${utilisateur.nom}" required style="width:100%; padding:8px 10px; border-radius:8px; border:1.5px solid var(--border-color); background:var(--input-bg); color:var(--text-primary);">
                </div>
                <div style="margin-bottom:14px;">
                    <label for="email-modif" style="font-weight:600; color:var(--text-primary);">Email</label><br>
                    <input type="email" id="email-modif" value="${utilisateur.email}" required style="width:100%; padding:8px 10px; border-radius:8px; border:1.5px solid var(--border-color); background:var(--input-bg); color:var(--text-primary);">
                </div>
                <div style="margin-bottom:18px;">
                    <label for="role-modif" style="font-weight:600; color:var(--text-primary);">Rôle</label><br>
                    <select id="role-modif" required style="width:100%; padding:8px 10px; border-radius:8px; border:1.5px solid var(--border-color); background:var(--input-bg); color:var(--text-primary);">
                        <option value="Utilisateur" ${utilisateur.role === 'Utilisateur' ? 'selected' : ''}>Utilisateur</option>
                        <option value="Admin" ${utilisateur.role === 'Admin' ? 'selected' : ''}>Admin</option>
                    </select>
                </div>
                <div style="text-align:right;">
                    <button type="button" onclick="fermerModalModifUtilisateur()" class="btn-annuler">Annuler</button>
                    <button type="submit" class="btn-create"><i class="fa fa-save"></i> Sauvegarder</button>
                </div>
            </form>
        </div>
    `;
    ...
    // Gestion du formulaire
    const form = modal.querySelector('#form-modif-user');
    form.onsubmit = function(e) {
        e.preventDefault();
        
        const nom = document.getElementById('nom-modif').value.trim();
        const email = document.getElementById('email-modif').value.trim();
        const role = document.getElementById('role-modif').value;
        ...
        if(nom && email && role) {
            ...
            sauvegarderUtilisateurs();
            mettreAJourTableauUtilisateurs();
            fermerModalModifUtilisateur();
        }
    };
}

function fermerModalModifUtilisateur() {
    const modal = document.getElementById('modal-modif-user');
    if(modal) {
        modal.remove();
    }
}
...
// Gestion dynamique des utilisateurs (admin)
const formAjoutUser = document.getElementById('form-ajout-user');
const usersTbody = document.getElementById('admin-users-tbody');
let usersList = [
  { nom: 'Sarah Dupont', email: 'sarah.dupont@email.com', role: 'Admin' },
  { nom: 'Jean Martin', email: 'jean.martin@email.com', role: 'Utilisateur' }
];
function renderUsersTable() {
  ...
  usersList.forEach((user, idx) => {
    ...
    tr.innerHTML = `<td>${user.nom}</td><td>${user.email}</td><td>${user.role}</td><td><button class='btn-danger' data-idx='${idx}'>Supprimer</button></td>`;
    usersTbody.appendChild(tr);
  });
  // Gestion suppression
  usersTbody.querySelectorAll('.btn-danger').forEach(btn => {
    ...
    graphiqueDiv.innerHTML = donnees.map(donnee => {
        ...
        return `
            <div style="display:flex; flex-direction:column; align-items:center; flex:1;">
                <div style="width:100%; background:linear-gradient(to top, #fbbf24, #f59e42); border-radius:4px 4px 0 0; min-height:4px; height:${hauteur}px;"></div>
...
function genererGraphiqueJours() {
    ...
    graphiqueDiv.innerHTML = donnees.map(donnee => {
        ...
        return `
            <div style="display:flex; flex-direction:column; align-items:center; flex:1;">
                <div style="width:100%; background:linear-gradient(to top, #eab308, #f59e42); border-radius:4px 4px 0 0; min-height:4px; height:${hauteur}px;"></div>
...
window.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialisation du bouton Ajouter un utilisateur à la toute fin
    const btnAjouter = document.getElementById('btn-ajouter-user');
    if(btnAjouter) {
        btnAjouter.onclick = function() {
            // Supprimer tout ancien formulaire
            let oldForm = document.getElementById('form-ajout-user-inline');
            if(oldForm) oldForm.remove();
            // Créer le formulaire juste sous le bouton
            const form = document.createElement('form');
            form.id = 'form-ajout-user-inline';
            form.style = 'margin:18px 0; display:flex; gap:12px; align-items:flex-end; flex-wrap:wrap; background:#fffbe6; border-radius:10px; padding:16px; box-shadow:0 2px 8px #fbbf2422;';
            form.innerHTML = `
                <div style=\"flex:1; min-width:120px;\">
                    <label style=\"color:#222; font-weight:600;\">Nom</label><br>
                    <input type=\"text\" id=\"user-nom-inline\" required style=\"width:100%; padding:6px 8px; border-radius:6px; border:1.5px solid #fbbf24; background:#fff; color:#222;\">
                </div>
                <div style=\"flex:1; min-width:120px;\">
                    <label style=\"color:#222; font-weight:600;\">Email</label><br>
                    <input type=\"email\" id=\"user-email-inline\" required style=\"width:100%; padding:6px 8px; border-radius:6px; border:1.5px solid #fbbf24; background:#fff; color:#222;\">
                </div>
                <div style=\"flex:1; min-width:100px;\">
                    <label style=\"color:#222; font-weight:600;\">Rôle</label><br>
                    <select id=\"user-role-inline\" required style=\"width:100%; padding:6px 8px; border-radius:6px; border:1.5px solid #fbbf24; background:#fff; color:#222;\">
                        <option value=\"Utilisateur\">Utilisateur</option>
                        <option value=\"Admin\">Admin</option>
                    </select>
                </div>
                <button type=\"submit\" class=\"btn-create\" style=\"font-size:0.95em; padding:8px 18px;\"><i class=\"fa fa-user-plus\"></i> Ajouter</button>
            `;
            btnAjouter.parentNode.insertBefore(form, btnAjouter.nextSibling);
            form.onsubmit = function(e) {
                e.preventDefault();
                const nom = document.getElementById('user-nom-inline').value.trim();
                const email = document.getElementById('user-email-inline').value.trim();
                const role = document.getElementById('user-role-inline').value;
                if(nom && email && role) {
                    const nouvelUtilisateur = {
                        id: Date.now(),
                        nom,
...
function mettreAJourTableauUtilisateurs() {
    const centreAdminSection = document.getElementById('centre-admin');
    if(!centreAdminSection) return;
    const tableBody = centreAdminSection.querySelector('tbody');
    if(!tableBody) return;
    tableBody.innerHTML = '';
    utilisateurs.forEach(utilisateur => {
        const row = document.createElement('tr');
        row.innerHTML = `
            <td><span class="user-nom">${utilisateur.nom}</span></td>
            <td><span class="user-email">${utilisateur.email}</span></td>
            <td><span class="user-role">${utilisateur.role}</span></td>
            <td>
                <button class="btn-copy btn-modif-user" data-id="${utilisateur.id}">Modifier</button>
                <button class="btn-copy btn-suppr-user" data-id="${utilisateur.id}" style="margin-left:8px; background:#ef4444; color:#fff; border-color:#ef4444;">Supprimer</button>
            </td>
        `;
        tableBody.appendChild(row);
    });
    // Ajout des listeners pour modifier et supprimer
    tableBody.querySelectorAll('.btn-suppr-user').forEach(btn => {
    ...
    tableBody.querySelectorAll('.btn-modif-user').forEach(btn => {
        btn.onclick = function() {
            const id = parseInt(this.getAttribute('data-id'));
            ...
            row.innerHTML = `
                <td><input type='text' value='${utilisateur.nom}' class='input-modif-nom' style='width:100%; padding:4px 8px; border-radius:6px; border:1.5px solid #fbbf24;'></td>
                <td><input type='email' value='${utilisateur.email}' class='input-modif-email' style='width:100%; padding:4px 8px; border-radius:6px; border:1.5px solid #fbbf24;'></td>
                <td><select class='input-modif-role' style='width:100%; padding:4px 8px; border-radius:6px; border:1.5px solid #fbbf24;'><option value='Utilisateur' ${utilisateur.role==='Utilisateur'?'selected':''}>Utilisateur</option><option value='Admin' ${utilisateur.role==='Admin'?'selected':''}>Admin</option></select></td>
                <td>
                    <button class='btn-create btn-save-user' style='font-size:0.95em; padding:6px 16px;'>Enregistrer</button>
                    <button class='btn-annuler btn-cancel-user' style='font-size:0.95em; padding:6px 16px; margin-left:8px;'>Annuler</button>
                </td>
            `;
            // Listener Enregistrer
            row.querySelector('.btn-save-user').onclick = function() {
                const newNom = row.querySelector('.input-modif-nom').value.trim();
                const newEmail = row.querySelector('.input-modif-email').value.trim();
                const newRole = row.querySelector('.input-modif-role').value;
            ...
            // Listener Annuler
            row.querySelector('.btn-cancel-user').onclick = function() {
                mettreAJourTableauUtilisateurs();
            };
        };
    });
